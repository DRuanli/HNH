...
public class experiment {
    ...
    public experiment(String databaseFile, String profitFile, String outputFile,
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 0) {
            System.err.println("Usage: java experiment <database> <profits> <k> <minProb> " +
                    "[output] [--no-parallel] [--debug]");
            System.err.println();
            System.err.println("Example:");
            System.err.println("  java experiment data/chess_db.txt data/chess_profits.txt 100 0.1 --debug");
            System.exit(1);
        }

        String db = "src/data/chess_database.txt", prof = "src/data/chess_profit.txt";
        int k = 10000; //Integer.parseInt(args[2]);
        double minP = 0.7;// Double.parseDouble(args[3]);
        String out = "experiment_output.txt";
        ...
        for (int i = 4; i < args.length; i++) {
            switch (args[i]) {
                ...
                case "--debug":
                    ...
                    break;
                default:
                    if (!args[i].startsWith("-")) out = args[i];
                    break;
            }
        }

        experiment algo = new experiment(db, prof, out, k, minP, par, dbg);
        experiment.MiningResult result = algo.run();
    ...
    private static UPUList buildUPUListFromEntries(Set<Integer> itemset,
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (e.logProbability > LOG_ONE_MINUS_EPS) {
                logComp = LOG_ZERO;
    ...
    private static UPUList joinTwoPointer(UPUList list1, UPUList list2,
        ...
        double[] rems = new double[maxCount];
        double[] probs = new double[maxCount];
    ...
    private void exploreExtensions(UPUList prefix, int startIndex) {
        ...
        for (int i = startIndex; i < itemCount; i++) {
            ...
            //if (extList.positiveUpperBound < threshold - EPSILON) continue;
    ...
    private void writeResults(MiningResult result) throws IOException {
        ...
        for (Pattern p : result.patterns) {
            sb.append(String.format(Locale.US, "%-6d %-40s %-15.4f %-15.6f%n",
                    rank++, p.sortedItems, p.expectedUtility, p.existentialProbability));
        }
    ...
    private static final class Transaction {
        ...
        double getProbability(int item) {
    ...
    private static final class TopKCollector {
        ...
        List<Pattern> getResults() {
            lock.lock();
            try {
                return new ArrayList<>(heap.descendingSet());
            } finally {
                lock.unlock();
            }
        }
    ...
    private final class EntryCollectionTask
        ...
        EntryCollectionTask(int from, int to) {
            this.from = from;
            this.to = to;
        }
    ...
    private final class DFSMiningTask extends RecursiveAction {
        ...
        DFSMiningTask(int start, int end) {
